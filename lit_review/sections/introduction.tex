...
% Gesture/interaction solutions
% -Specific hardware
% -Thumb gestures (overloading touch input)
% Any study compare to touch or assess learnability?
...
% Research Question/s:
% - Enhancing usability of HMDs through combination of Gaze Tracking, Head-Tracking/Gestures, and touch input
% - Evaluating the usability of an interface for a HMD that takes input from Gaze Tracking, Head-Tracking/Gestures, and touch input.
% - Evaluating an adaptive UI (for a HMD) which reacts to changes between relative positioning of the HMD and user's head
% - Expand upon existing works which utilise head-tracking to enhance one-handed HMD interactions by distinguishing between head vs phone movement
% ^ Outcome of the intro (i.e. have a discussion that builds-up to the question), or Intro inception (i.e. what we already want to do and then go on to explain / deconstruct)?
% Actually list these?
